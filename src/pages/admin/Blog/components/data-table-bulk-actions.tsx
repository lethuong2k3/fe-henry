...
import { DataTableBulkActions as BulkActionsToolbar } from '@/components/admin/data-table'
...
export function DataTableBulkActions<TData>({
  ...
  const handleBulkStatusChange = (status: string) => {
    const selectedTasks = selectedRows.map((row) => row.original as Blog)
          ...
          <DropdownMenuContent sideOffset={14}>
            {actives.map((active, index) => (
              <DropdownMenuItem
                ...
                onClick={() => handleBulkPriorityChange(active.value)}
              >
                {active.icon && (
                  <active.icon className='text-muted-foreground size-4' />
                )}
                {active.label}
              </DropdownMenuItem>
            ))}
